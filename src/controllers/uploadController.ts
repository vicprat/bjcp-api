...
export const upload = async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).send('Datos subidos exitosamente')
  } catch (error) {
    if (error instanceof Error) {
      res.status(500).send(`Error al subir los datos: ${error.message}`)
    } else {
      res.status(500).send('Error desconocido al subir los datos')
    }
  }
}
