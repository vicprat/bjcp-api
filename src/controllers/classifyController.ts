...
export const classify = (req: Request, res: Response) => {
  try {
    ...
    res.status(200).send('Archivos creados exitosamente')
  } catch (error) {
    if (error instanceof Error) {
      res.status(500).send(`Error al clasificar: ${error.message}`)
    } else {
      res.status(500).send('Error desconocido al clasificar')
    }
  }
}
